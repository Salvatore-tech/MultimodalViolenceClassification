    ...
    if '_label_A' not in name:
        for gt_line in gt_lines:
            if name in gt_line:
                ...
                break
    mod = (lens) % clip_len # minusing 1 is to align flow  rgb: minusing 1 when extracting features
    gt_vec = gt_vec[:-1]
    if mod:
        gt_vec = gt_vec[:-mod]
    gt.extend(gt_vec)
