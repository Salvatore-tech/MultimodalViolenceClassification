...
if __name__ == '__main__':
    ...
    gt = np.load(args.gt)
    st = time.time()
    pr_auc, pr_auc_online, pred, pred2 = test(test_loader, model, device, gt)
    print('Time:{}'.format(time.time()-st))
    print('offline pr_auc:{0:.4}; online pr_auc:{1:.4}\n'.format(pr_auc, pr_auc_online))
    np.save("np_predictions/test/off_hl-net.npy", pred)
    np.save("np_predictions/test/on_hl-net.npy", pred2)